    ...
    cognitoidentityserviceprovider.getUser(params, function (err, data) {
        if (err) {
            ...
            for (var i = 0; i < UA.length; i++) {
                if (UA[i].Name == 'sub') principalId = UA[i].Value;
                if (UA[i].Name == 'email') email = UA[i].Value;
                if (UA[i].Name == 'email_verified') verified = UA[i].Value;
            }

            //if there is no email or unveried then unauthorize
            if (!verified || email == '') {
